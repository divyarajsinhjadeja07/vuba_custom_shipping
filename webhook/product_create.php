...
//file_put_contents($logs_dir . '/webhook_product_create.log', date('Y-m-d H:i:s') . ': ' . $webhook_content . PHP_EOL, FILE_APPEND);
...
//file_put_contents($logs_dir . '/webhook_debug.log', date('Y-m-d H:i:s') . ': Processing create for shop: ' . $shop_url . PHP_EOL, FILE_APPEND);
...
//file_put_contents($logs_dir . '/webhook_debug.log', date('Y-m-d H:i:s') . ': Creating product ID: ' . $product_id . PHP_EOL, FILE_APPEND);
...
foreach ($webhook_data['variants'] as $variant) {
    ...
    //file_put_contents($logs_dir . '/webhook_debug.log', date('Y-m-d H:i:s') . ': Final weight value: ' . $weight . ' ' . (isset($weight_unit) ? $weight_unit : 'kg') . PHP_EOL, FILE_APPEND);
    ...
    // Check if product already exists (it shouldn't for create, but just in case)
